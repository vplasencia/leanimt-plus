...
///
/// The full node table is stored on-chain (like the Indexed Merkle Tree) rather
...
/// physical index of the low leaf and the library validates it in O(1), exactly as
/// the Indexed Merkle Tree does. This keeps every mutation at O(depth) storage
...
library InternalLeanIMTPlus {
    ...
    function _verifyProof(LeanIMTPlusProof memory proof) internal pure returns (bool) {
        ...
        if (
            value >= SNARK_SCALAR_FIELD ||
            leafValue >= SNARK_SCALAR_FIELD ||
            leafNext >= SNARK_SCALAR_FIELD
        ) {
            return false;
        }

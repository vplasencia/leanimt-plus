...
LeanIMT+ extends the [LeanIMT](https://github.com/privacy-scaling-explorations/zk-kit.solidity/tree/main/packages/lean-imt)
with the indexed-leaf design of the
[Indexed Merkle Tree](https://github.com/dl-solarity/solidity-lib/blob/master/contracts/libs/data-structures/IndexedMerkleTree.sol),
so it can prove that a value is **not** in the tree without revealing the full
leaf set and without the fixed, deep trees that Sparse Merkle Trees require.
...
```solidity
import {LeanIMTPlus} from "leanimt-plus.sol/LeanIMTPlus.sol";
import {LeanIMTPlusData, LeanIMTPlusProof} from "leanimt-plus.sol/InternalLeanIMTPlus.sol";
...
Approximate gas at `n = 64` leaves (`depth = 7`), dominated by on-chain Poseidon:

| Operation                      | Gas   |
| ------------------------------ | ----- |
| `insert`                       | ~988k |
| `update` (in place)            | ~857k |
| `remove`                       | ~764k |
| `verifyProof` (membership)     | ~535k |
| `verifyProof` (non-membership) | ~535k |

Cost scales with `depth = ceil(log2(n))`. In zero-knowledge settings, proof
verification is done in-circuit rather than on-chain, so the on-chain
`verifyProof` cost applies only to fully on-chain verifiers.
